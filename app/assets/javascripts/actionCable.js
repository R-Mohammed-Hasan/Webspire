(function(global, factory) {
    ...
    var logger = {
        log: function log() {
            if (this.enabled) {
                ...
                for (
                    var _len = arguments.length, messages = Array(_len), _key = 0; _key < _len; _key++
                ) {
                    messages[_key] = arguments[_key];
                }
                messages.push(Date.now());
                (_adapters$logger = adapters.logger).log.apply(
                    _adapters$logger, ["[ActionCable]"].concat(messages)
                );
            }
        },
    };
    ...
    var Connection = (function() {
        ...
        Connection.prototype.isState = function isState() {
            for (
                var _len = arguments.length, states = Array(_len), _key = 0; _key < _len; _key++
            ) {
        ...
        Subscriptions.prototype.notifyAll = function notifyAll(callbackName) {
            ...
            return this.subscriptions.map(function(subscription) {
                return _this3.notify.apply(
                    _this3, [subscription, callbackName].concat(args)
                );
            });
        };
