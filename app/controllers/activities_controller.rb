class ActivitiesController < ApplicationController
  ...
  def accept
    ...
    request[0].destroy
    Activity.create(user_id: @current_user.id, sender_id: params[:id],activity: "has accepted your friend request",status: "not_seen")
  ...
  def reject
    request = FollowRequest.where(sender_id: params[:id],receiver_id: @current_user.id)
    request[0].destroy
    Activity.create(user_id: @current_user.id, sender_id: params[:id],activity: "has rejected your friend request",status: "not_seen")
